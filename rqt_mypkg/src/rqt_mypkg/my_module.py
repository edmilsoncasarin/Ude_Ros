...
from ude_arduino.msg import ArmJointState
...
class MyPlugin(Plugin):
    ...
    def __init__(self, context):
        ...
        #os.system('roslaunch ude_moveit_config demo.launch')
	...
	rospy.Subscriber("/ude_test/joint_states", JointState, self.joint_states_callback)

        img = QPixmap('/home/edmilson/catkin_ws/src/ude_test/rqt_mypkg/resource/dorna.png')
        ...
        self.arr_sl = [self._widget.SlJoint1,self._widget.SlJoint2,self._widget.SlJoint3,self._widget.SlJoint4]
        self.arr_ShowSl = [self._widget.ShowJoint1,self._widget.ShowJoint2,self._widget.ShowJoint3,self._widget.ShowJoint4]
        ...
        lista_arq = ''
        for i in os.listdir("/home/edmilson/trajectories_centauri6dof"): lista_arq+=(i+'\n')
        print(lista_arq)
        ...
        box_pick = [0, 700, 000, 0]
        box_move = [0, 1200, 1400, 0]
        box_place = [800, 330, 1600, 0]

        banana_pick = [8000, 3735, 13600, 0, gripper['banana']]
        banana_move = [8000, 1412, 6600, 80, gripper['banana']]
        banana_place = [3555, 2733, 13399, 0, gripper['open']]
	...
	self._widget.SlJoint1.setMaximum(90)
        self._widget.SlJoint1.setMinimum(-90)
        self._widget.SlJoint2.setMaximum(50)
        self._widget.SlJoint2.setMinimum(-90)
	self._widget.SlJoint3.setMaximum(0)
        self._widget.SlJoint3.setMinimum(-90)
        self._widget.SlJoint4.setMaximum(-5)
        self._widget.SlJoint4.setMinimum(-90)
    ...
    def joint_states_callback(self, joint_state):
        ...
        #goal.position5 = self.goal.position5
        pub.publish(goal)
    ...
    def _Send_joints_teleoperation(self):
        ...
        self.goal.position1 = np.int16(((self.arr_sl[0].value()*np.pi)/180)*(1200/(2*np.pi)))
        self.goal.position2 = np.int16(((self.arr_sl[1].value()*np.pi)/180)*(3200/(2*np.pi)))
        self.goal.position3 = np.int16(((self.arr_sl[2].value()*np.pi)/180)*(3200/(2*np.pi)))
        self.goal.position4 = np.int16(((self.arr_sl[3].value()*np.pi)/180)*(2000/(2*np.pi)))
    ...
    def _fcn_gripper(self):
        ...
        if self.grip == 1:
            self.goal.position5 = 100
            self.pub2.publish(self.goal)
            self._widget.ShowText.setText("Gripper ON")
        else:
            self.goal.position5 = 10
            self.pub2.publish(self.goal)
    ...
    def _save_pose(self):
        self.GoalPosition = [self.goal.position1, self.goal.position2, self.goal.position3, self.goal.position4,self.goal.position5]
        ...
        self._widget.ShowText.setText(
            ...
            "\nJoint1:   "+ str(round((self.joint_visualizer[0]*360)/1200))+" degrees"+
            "\nJoint2:   "+ str(round((self.joint_visualizer[1]*360)/3200))+" degrees"+         
            "\nJoint3:   "+ str(round((self.joint_visualizer[2]*360)/3200))+" degrees"+ 
            "\nJoint4:   "+ str(round((self.joint_visualizer[3]*360)/2000))+" degrees"+            
            "\nGripper:  "+ str(self.joint_visualizer[4])+" degrees"
            )
    ...
    def _execute_path(self):
        ...
        for num_array_pose in self.trajectory:
            ...
            joint_goal[0] = (float(num_array_pose[0])*(2*np.pi))/1200
            goal.position2 = np.int16(num_array_pose[1])
            joint_goal[1] = (float(num_array_pose[1])*(2*np.pi))/3200
            goal.position3 = np.int16(num_array_pose[2])
            joint_goal[2] = (float(num_array_pose[2])*(2*np.pi))/3200
            goal.position4 = np.int16(num_array_pose[3])
            joint_goal[3] = (float(num_array_pose[3])*(2*np.pi))/2000
